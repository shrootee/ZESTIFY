...
function Card({ 
  ...
  const [commentsCount, setCommentsCount] = useState(reel?.commentsCount || 0); // ✅ Add this state

  // ✅ Listen for comment count updates from Firestore
  useEffect(() => {
    if (reel?.commentsCount !== undefined) {
...
<button 
  onClick={(e) => {
    e.stopPropagation();
    console.log("Button clicked! Current state:", isCommentOpen); // ✅ Add this log
    setIsCommentOpen(true);
    console.log("State set to true"); // ✅ Add this log
  }}
  className="flex flex-col z-100 items-center gap-1 group"
>
  <div
    className={`w-10 h-10 md:w-11 md:h-11 ${
  ...
  >
    ...
    isHomepage ? 'text-sm' : 'text-base md:text-lg'
  } mb-1 ${isHomepage ? 'pr-10' : 'pr-14 md:pr-16'} ${
    isHomepage ? 'truncate' : ''
  }`}>
    {reel.foodName}
  </h4>
              ...
              {isHomepage ? (
    <p 
      className="text-white/80 text-xs md:text-sm mb-2 max-w-[65%] md:max-w-[60%]"
      style={{ 
        height: '20px',
        lineHeight: '20px',
        overflow: 'hidden',
  ...
  >
    ...
    ORDER NOW
      ...
      {/* ✅ ADD COMMENT SECTION HERE - Right before the closing fragment */}
      <CommentSection 
        reelId={reel.id}
        currentUser={user}
